...
func TestExecBasic(t *testing.T) {
	if _, err := exec.LookPath("qbec-replay-http-server"); err != nil {
		t.SkipNow()
	}
	a := assert.New(t)
	ds, err := FromURL("exec-http://replay?exe=qbec-replay-http-server")
...
func TestExecNegative(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "exe-err",
			url:  "exec-http://replay?exe=qbec-replay-http-server",
			path: "/fail",
			asserter: func(t *testing.T, resolved string, err error) {
				require.Error(t, err)
		...
		{
			name: "exe-slow",
			url:  "exec-http://replay?exe=qbec-replay-http-server&request-timeout=100ms",
			path: "/slow",
			asserter: func(t *testing.T, resolved string, err error) {
				require.Error(t, err)
