...
func addForceOptions(cmd *cobra.Command, prefix string) func() forceOptions {
	var f forceOptions
	ctxUsage := fmt.Sprintf("force K8s context with supplied value. Special values are %s and %s for in-cluster and current contexts respectively. Default value can be overridden by setting QBEC_FORCE_K8S_CONTEXT=<context>",
	...
	nsUsage := fmt.Sprintf("override default namespace for environment with supplied value. The special value %s can be used to extract the value in the kube config. Default value can be overridden by setting QBEC_FORCE_K8S_NAMESPACE=<ns>", currentMarker)
