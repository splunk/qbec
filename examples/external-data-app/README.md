...
Possibly the world's most stupid way to create a config map.
...
* the computed variable called `cmdConfig` in [qbec.yaml](qbec.yaml) that shows the JSON that can be used to 
  configure how an external program is invoked with arguments, environment variables and standard input.
* the data source definition in [qbec.yaml](qbec.yaml) that defines a data source called `config-map` and associates the
  command configuration with it.
